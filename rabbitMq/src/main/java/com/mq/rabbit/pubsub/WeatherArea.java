...
//        channel.queueDeclare(RabbitConstant.EXCHANGE_WEATHER,false,false,false,null);
        ...
        areaMap.entrySet().forEach(item ->{
            ...
            try {
                // 交换机名，队列名，，队列信息
                channel.basicPublish(RabbitConstant.EXCHANGE_WEATHER,"",null,item.getValue().getBytes());
            } catch (IOException e) {
                e.printStackTrace();
            }
        });
